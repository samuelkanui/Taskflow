...
import { Plus, Tag as TagIcon, Trash2, X, Sparkles, Edit2, Tags, Hash } from 'lucide-react';
                ...
                <div className="mx-auto max-w-[95%] 2xl:max-w-[1600px] space-y-8 p-6 lg:p-8">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-2 flex items-center gap-2 text-lg text-white/90">
                                            ...
                                            {tags.length} tag{tags.length !== 1 ? 's' : ''} • Label and organize tasks
                                        </p>
                                    </div>
                                </div>
                                <Button 
                    ...
                    {tags.length === 0 ? (
                        <div className="relative overflow-hidden rounded-2xl border-2 border-dashed border-gray-300 bg-gradient-to-br from-gray-50 to-white p-16 text-center shadow-lg dark:border-gray-700 dark:from-gray-800 dark:to-gray-900">
                            ...
                            <div className="relative">
                                ...
                                <p className="mb-6 text-gray-600 dark:text-gray-400">
                                    Create your first tag to label and organize your tasks efficiently!
                                </p>
                                <Button 
                                    onClick={() => setShowModal(true)} 
                        ...
                        {tags.map((tag) => (
                            ...
                            >
                                <div className="absolute right-0 top-0 h-full w-1/3 bg-gradient-to-l from-white/30 to-transparent dark:from-black/30"></div>
                                <div className="mb-3 flex items-center justify-between">
                                    <div
                                        className="flex size-10 items-center justify-center rounded-lg"
                                        style={{ backgroundColor: tag.color + '20' }}
                                    >
                                        <TagIcon className="size-5" style={{ color: tag.color }} />
                                    </div>
                                    <div className="relative z-10 flex gap-1">
                                        <button
                                            onClick={() => startEdit(tag)}
                                            className="rounded-lg bg-gradient-to-br from-blue-50 to-indigo-50 p-1.5 text-blue-600 opacity-0 shadow-md transition-all hover:scale-110 hover:shadow-lg group-hover:opacity-100 dark:from-blue-900/30 dark:to-indigo-900/30 dark:text-blue-400"
                                        >
                                            <Edit2 className="size-3.5" />
                                        </button>
                                        <button
                                            onClick={() => deleteTag(tag.id)}
                                            className="rounded-lg bg-gradient-to-br from-red-50 to-rose-50 p-1.5 text-red-600 opacity-0 shadow-md transition-all hover:scale-110 hover:shadow-lg group-hover:opacity-100 dark:from-red-900/30 dark:to-rose-900/30 dark:text-red-400"
                                        >
                                ...
                                <div className="relative z-10 flex items-center justify-between rounded-lg p-2 text-xs" style={{ backgroundColor: tag.color + '20' }}>
                                    <span className="font-semibold" style={{ color: tag.color }}>Tasks</span>
                                    <span
                                        ...
                                        {colors.map((color) => (
                                            <button
                                                ...
                                                className={`size-12 rounded-xl shadow-md transition-all hover:scale-110 hover:shadow-lg ${
                                                    data.color === color ? 'ring-4 ring-offset-2' : ''
                                                }`}
                                                style={{
                                                    backgroundColor: color,
                                                    ...(data.color === color ? { boxShadow: `0 0 0 4px ${color}40` } : {})
                                                }}
                                            />
                                        ))}
                                    </div>
                                    <div className="mt-3">
                                        <Label htmlFor="custom_color" className="text-xs">Or enter custom color</Label>
