...
import { CheckCircle2, Circle, Clock, Copy, Edit, Folder, Plus, Search, Tag as TagIcon, ListChecks, Filter } from 'lucide-react';
...
export default function TasksIndex() {
    const { tasks, categories, tags, goals, filters } = usePage<TasksPageProps>().props;
    ...
    const toggleComplete = (taskId: number) => {
        router.post(`/tasks/${taskId}/toggle-complete`, {}, {
            preserveScroll: true,
        });
    };
                ...
                <div className="mx-auto max-w-[95%] 2xl:max-w-[1600px] space-y-8 p-6 lg:p-8">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-1 text-lg text-white/90">
                                            {tasks.total} tasks • Organize and track your work
                                        </p>
                                    </div>
                                </div>
                                <Link
                    ...
                    <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
                        ...
                        <select
                            ...
                            onChange={(e) => handleFilterChange('view', e.target.value === 'all' ? '' : e.target.value)}
                        ...
                        <select
                            ...
                            onChange={(e) => handleFilterChange('priority', e.target.value === 'all' ? '' : e.target.value)}
                            className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        >
                        ...
                        <select
                            ...
                            onChange={(e) => handleFilterChange('goal_id', e.target.value === 'all' ? '' : e.target.value)}
                            className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        >
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-start justify-between gap-4">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <span
                                                    className={`whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-bold uppercase tracking-wide ${getPriorityColor(task.priority)}`}
                                                >
                                                    {task.priority}
                                                </span>
                    ...
                    {tasks.last_page > 1 && (
                        <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white px-6 py-4 shadow-lg dark:border-gray-700 dark:bg-gray-800">
                            <div className="text-sm font-medium text-gray-700 dark:text-gray-300">
                                Showing <span className="font-bold text-purple-600 dark:text-purple-400">{tasks.from}</span> to <span className="font-bold text-purple-600 dark:text-purple-400">{tasks.to}</span> of <span className="font-bold text-purple-600 dark:text-purple-400">{tasks.total}</span> tasks
                            </div>
                            <div className="flex gap-2">
                                {tasks.current_page > 1 && (
                                    <Link
                                        href="/tasks"
                                        data={{ ...filters, page: tasks.current_page - 1 }}
